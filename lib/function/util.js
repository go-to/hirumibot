 ...
 */
exports.updateHolidayListCsv = async () => {
    // CSV取得
    const url = hirumiConst.HOLIDAY_LIST_CSV_PROVIDER_URL;
    const content = await request.get(url).responseType('blob');
    const savePath = `${hirumiConst.TMP_DIR}/${hirumiConst.HOLIDAY_LIST_FILE_CAO_CSV}`;
    if (!content.hasOwnProperty('body')) {
        return false;
    }

    // 文字コード変換
    const detected = encoding.detect(content.body);
    const sjisStr = iconv.decode(content.body, detected);
    const utf8Str = iconv.encode(sjisStr, hirumiConst.CHARSET).toString();
    await fs.writeFileSync(savePath, this.replaceLineFeed(utf8Str));

    // ファイル差し替え
    const today = this.getToday();
    const holidayListFilePath = `${hirumiConst.HOLIDAY_LIST_DIR}/${hirumiConst.HOLIDAY_LIST_FILE_CAO_CSV}`;
    if (await this.isExistFile(holidayListFilePath)) {
        await fs.copyFileSync(holidayListFilePath, `${holidayListFilePath}.${today}`);
    }
    await fs.copyFileSync(savePath, holidayListFilePath);
