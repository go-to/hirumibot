 ...
 */
exports.createFile = (filePath, text) => {
...
exports.readFile = (filePath) => {
    let content = fs.readFileSync(filePath, hirumiConst.CHARSET);
    if (content === '') {
        return [];
    }
 ...
 */
exports.findWord = (text, filePath) => {
    // TODO 処理をスマート化したい
 ...
 */
exports.writeNameToFile = (name, filePath) => {
    const wasFound = this.findWord(name, filePath);
    // すでに追加されている場合は終了
    if (wasFound) {
        return hirumiConst.RESULT_CODE_UNNECESSARY;
    }
    let nameList = this.readFile(filePath);
    nameList.push(name);
 ...
 * @return {number} 処理結果に応じてコードを返す
 */
exports.overwriteNameToFile = (data, filePath) => {
