...
exports.checkSchedule = async () => {
    ...
    // 祝日判定
    let isHoliday = false;
    if (hirumiConst.ENABLE_HOLIDAY) {
        const today = hirumiUtil.getToday('/', false);
        // 祝日CSVに含まれるかチェック
        // TODO 現状はCSVを内部で抱えているが、内閣府のサイトから定期的に自動取得するようにしたい
        let holidayListFilePath = `${hirumiConst.HOLIDAY_LIST_DIR}/${hirumiConst.HOLIDAY_LIST_FILE_CAO_CSV}`;
        isHoliday = await hirumiUtil.findWordCsv(today, holidayListFilePath);
        if (!isHoliday) {
            // 独自に定義した休日チェック（年末年始や組織固有の記念日等を想定）
            holidayListFilePath = `${hirumiConst.HOLIDAY_LIST_DIR}/${hirumiConst.HOLIDAY_LIST_FILE_ORIGINAL}`;
            isHoliday = await hirumiUtil.findWordCsv(today, holidayListFilePath);
        }
    }
    ...
    for (idx in days) {
        ...
        // 祝日と判定された場合は処理を実行しない
        if (isHoliday && targetDay !== hirumiConst.WORD_LIST_FILE_AUTO_POST_EVERYDAY) {
            let needExecution = true;
            // 毎日・土曜日・日曜日以外は実行不要と判断する
            switch (currentDay) {
...
exports.searchAutoPostWord = async (conversationId, text) => {
    ...
    // 投稿日時
    let hourMinute = autoPostScheduleText.match(/[0-9]+時[0-9]+分|[0-9]+時|[0-9]+:[0-9]+/g);
    if (hourMinute === null) {
